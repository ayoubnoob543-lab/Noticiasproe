/*
# NoticiasPro — Database Schema for Automated News Platform

## Overview
Creates the complete schema for a professional news platform with automated
news collection from multiple sources (RSS, NewsAPI, GNews, etc.), AI rewriting,
duplicate detection, comments, newsletter subscriptions, and error logging.

## New Tables

### 1. `categories`
- Stores news categories (Fútbol, Tecnología, etc.)
- `id` (uuid, PK), `slug` (text, unique), `name`, `description`, `color`, `icon`,
  `parent_id` (self-reference for subcategories), `priority` (int), `created_at`

### 2. `authors`
- Stores article authors (real journalists + AI-generated profiles)
- `id` (uuid, PK), `name`, `avatar`, `bio`, `role`, `created_at`

### 3. `sources`
- News sources (RSS feeds, NewsAPI, GNews, TheNewsAPI, Mediastack, Currents,
  Guardian, NYT, Bing News, Google News, custom feeds)
- `id` (uuid, PK), `name`, `url`, `type`, `category`, `priority` (int),
  `is_active` (bool), `language`, `last_fetched_at`, `fetch_interval_seconds`,
  `articles_fetched` (int counter), `errors_count` (int), `config` (jsonb for API keys/params),
  `created_at`, `updated_at`

### 4. `articles`
- Published news articles (rewritten by AI)
- `id` (uuid, PK), `slug` (unique), `title`, `subtitle`, `summary`, `content`,
  `excerpt`, `meta_description`, `image`, `image_alt`, `category_id` (FK),
  `category_slug`, `subcategory_slug`, `tags` (text[]), `author_id` (FK),
  `published_at`, `updated_at`, `reading_time` (int), `views` (int),
  `comments_count` (int), `shares` (int), `is_breaking`, `is_featured`,
  `is_trending`, `status` (draft/published/scheduled), `source_id` (FK),
  `source_url`, `source_title_hash` (for duplicate detection), `ai_rewritten` (bool),
  `original_language`, `created_at`
- Indexes on slug, category_slug, published_at, status, is_breaking, is_featured

### 5. `raw_articles`
- Raw articles fetched from sources before AI rewriting (queue table)
- `id` (uuid, PK), `source_id` (FK), `title`, `content`, `url`, `image`,
  `published_at`, `fetched_at`, `status` (pending/processing/published/duplicate/failed),
  `title_hash` (for dedup), `processed_at`, `error_message`
- Index on status, source_id, title_hash

### 6. `comments`
- User comments on articles
- `id` (uuid, PK), `article_id` (FK), `author_name`, `author_avatar`,
  `content`, `likes` (int), `parent_id` (self-ref for replies), `is_approved`,
  `created_at`

### 7. `newsletter_subscribers`
- Email subscribers to the newsletter
- `id` (uuid, PK), `email` (unique), `preferences` (text[]), `is_active`,
  `subscribed_at`

### 8. `fetch_errors`
- Error log for source fetching / AI processing
- `id` (uuid, PK), `source_id` (FK nullable), `type`, `message`, `severity`,
  `is_resolved`, `timestamp`, `resolved_at`

### 9. `teams`
- Football teams for the football section
- `id` (uuid, PK), `name`, `short_name`, `slug` (unique), `league`, `logo`,
  `colors`, `stadium`, `manager`, `position`, `played`, `won`, `drawn`, `lost`,
  `points`, `created_at`

### 10. `matches`
- Football matches
- `id` (uuid, PK), `home_team`, `away_team`, `home_score`, `away_score`,
  `date`, `status`, `competition`, `venue`, `created_at`

## Security
- RLS enabled on ALL tables.
- This is a no-auth public news site: articles, categories, authors, teams,
  matches, approved comments, and active sources are readable by anon.
- Writes (comments, newsletter subscriptions) are open to anon with validation.
- Admin operations (managing sources, articles, errors) use the service role key
  from edge functions, bypassing RLS.
*/
